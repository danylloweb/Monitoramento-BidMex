...
const URL_OLD_BUSCA = Env.get('URL_OLD_BUSCA', 'https://buscaaereo.com.br/#/');
const EMAIL = Env.get('EMAIL', 'danilopedrosa@mangue3.com');
const PASS_MANGUE3 = Env.get('PASS_MANGUE3', 'lourivaldoviado');
 ...
 * @returns {Promise<void>}
 */
const go = async function(page) {
  console.log('Entrou go');
  await page.goto(URL_OLD_BUSCA);
};
 ...
 * @returns {Promise<void>}
 */
const openSearch = async function (page) {
 ...
 * @returns {Promise<void>}
 */
const basics = async function (page, config) {
  await go(page);
  await login(page);
